...
const Benefits = () => (
  <section className="flex flex-col items-center bg-[#FaFaf4] px-4 py-8 lg:p-16">
    <h2 className={`mb-10 text-center text-3xl font-semibold md:text-4xl xl:text-5xl`}>
      Why Choose Us
    </h2>
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4 2xl:gap-10">
      {benefitsList.map((benefit, index) => (
        ...
        >
          <div className="absolute inset-0 flex translate-x-0 items-center justify-center transition-all delay-100 duration-300 ease-in-out group-hover:-translate-x-4 group-hover:opacity-0">
            <h3 className="p-8 text-lg font-semibold tracking-tight lg:text-xl">
              {benefit.title.toUpperCase()}
            </h3>
          </div>
          <div className="absolute inset-0 flex translate-x-8 flex-col items-center justify-center opacity-0 transition-all delay-100 duration-300 ease-in-out group-hover:translate-x-0 group-hover:opacity-100">
            <p className="px-4 text-gray-700">{benefit.description}</p>
          </div>
        </div>
      ))}
    </div>
